...
async def propose(session, trade):
    ...
    tracker[s + "_" + str(int(time.time()))] = {
        "symbol": s, "entry": px, "decision": "pending",
        "proposed_at": time.time(), "result": None
    }
    log.info("Pump: " + s + " x" + str(spike))

async def check_results(session):
    now = time.time()
    for key, t in list(tracker.items()):
        if t["decision"] == "pending" or t["result"] is not None:
            continue
        ...
        try:
            ...
            if decision == "approved":
                result_text = "+$" + str(profit) + " PROFIT" if pct > 0 else "$" + str(profit) + " LOSS"
            else:
                if pct > 0:
                    result_text = "Would have made +$" + str(profit)
                else:
                    result_text = "Good call! Saved $" + str(abs(profit))
            msg = (
                "<b>RESULT (30 min)</b>\n"
                + t["symbol"] + ": " + str(pct) + "%\n"
...
async def handle_cb(session, cb):
    ...
    if data.startswith("approve_"):
        ...
        if "orderId" in result:
            await send(session, "BOUGHT: " + s + "\nOrder: " + str(result["orderId"]) + "\nResult in 30 min")
        else:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            await send(session, "IP: " + my_ip)
        except:
            pass
        await send(session, "<b>PUMP DETECTOR LIVE!</b>\nScanning every 60 sec\nAmount: $" + str(TRADE_AMOUNT))
            ...
            if isinstance(t, dict):
                ...
                if s.endswith("USDT"):
                    try:
                        ...
                        pass
        log.info("Baseline: " + str(len(volume_history)) + " pairs")
